...
public:
    static void udpRegister(const int& fd, sockaddr*& serverAddrp) {
        ...
        if (!isValidString(account)) {
            showPrevious("Account cannot contain <space> / <tab>");
            return;
        }
        strcpy(password, getpass("Password: "));
        trimNewLine(password);
        if (!isValidString(password)) {
            showPrevious("Password cannot contain <space> / <tab>");
            return;
        }
    ...
    static void udpDeleteAccount(const int& fd, sockaddr*& serverAddrp) {
        ...
        trimNewLine(buffer);
        if (std::string(buffer) == "yes" || std::string(buffer) == "y" || std::string(buffer) == "Yes" || std::string(buffer) == "Y") {
    ...
    static void udpSetProfile(const int& fd, sockaddr*& serverAddrp) {
        ...
        if (!isValidString(name)) {
            showPrevious("Cannot contain <space> / <tab>");
            return;
        }
    ...
    static void udpAddArticle(const int& fd, sockaddr*& serverAddrp) {
        ...
        cleanScreen();
        printf("New Article\n");
        printf("Set permission( 1:public, 2:only me, 3:friends, 4:specific ): ");
        while (true) {
            if (fgets(buffer, MAXN, stdin) == NULL) {
                showPrevious();
                return;
            ...
            if (std::string(buffer) == "1") {
                ...
                msg = msg + " " + buffer;
                printf("Please enter specific accounts(one name per line) who can view this article:\n");
    ...
    static void udpDeleteArticle(const int& fd, sockaddr*& serverAddrp) {
        ...
        if (std::string(userConfirm) != "yes" && std::string(userConfirm) != "Yes" && std::string(userConfirm) != "Y" && std::string(userConfirm) != "y") {
            showPrevious("CANCELED");
            return;
        }
    ...
    static void udpChatGroup(const int& fd, sockaddr*& serverAddrp, const std::string& config) {
        ...
        while (true) {
            ...
            if (!isValidString(groupname)) {
                printf("Group name cannot contain <space> / <tab> \n");
            }
            else {
                break;
            }
